...
class MambaLayer(nn.Module):
    def __init__(self, dim, d_state=8, d_conv=3, expand=1, num_slices=None, layer_scale=None):
        super().__init__()
        self.dim = dim
...
class ConvBlock3D(nn.Module):
    """3D ConvNet block inspired by MambaVision's ConvBlock"""
    def __init__(self, dim, drop_path=0., layer_scale=None, kernel_size=3):
        super().__init__()
        
        self.conv1 = nn.Conv3d(dim, dim, kernel_size=kernel_size, stride=1, padding=kernel_size//2)
        self.norm1 = nn.BatchNorm3d(dim, eps=1e-5)
        self.act1 = nn.GELU()
        
        self.conv2 = nn.Conv3d(dim, dim, kernel_size=kernel_size, stride=1, padding=kernel_size//2)
...
class MambaEncoder(nn.Module):
    def __init__(self, in_chans=1, depths=[2, 2, 2, 2], dims=[48, 96, 192, 384],
        ...
        cur = 0
        for i in range(4):
            # Use ConvBlocks for early stages (higher resolution), Mamba for later stages
            use_conv = (i == 0 or i == 1)  # First two stages use conv blocks
            
            if use_conv:
                stage = nn.Sequential(
                    *[ConvBlock3D(dim=dims[i], 
                                  drop_path=dpr[cur + j], 
                ...
                stage = nn.Sequential(
                    *[MambaLayer(dim=dims[i], 
                                d_state=8,  # MambaVision uses d_state=8
                                d_conv=3,   # MambaVision uses d_conv=3
                                expand=1,   # Use expand=1 for efficiency with multi-directional scanning
    ...
    def forward_features(self, x):
        ...
        for i in range(4):
            ...
            # Apply GSC only for Mamba stages (later stages)
